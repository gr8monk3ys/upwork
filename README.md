...
pulling changes; `uv tool install --editable .` if you would rather it track
your working copy.
...
Note that `uv sync` and `pip install -e` put `upwork` in `.venv/bin`, **not**
on your PATH — you need the venv activated, or the full path
`.venv/bin/upwork`. Only `uv tool install` gives you a bare `upwork` command.
